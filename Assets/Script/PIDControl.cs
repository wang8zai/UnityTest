...
public static class PIDControl{
    ...
    public static void SpeedXPID(float target, Rigidbody2D rd, float PID_P, float PID_I, float PID_D, float deltaTime)
    {
...
//        Debug.Log("XSPEEDFORCE :" + Xspeedforce);
        rd.AddForce(new Vector3(Xspeedforce * deltaTime, 0.0f, 0.0f));
    }

    public static void PositionYPID(float target_Y, Rigidbody2D rd, float PID_P, float PID_I, float PID_D, float deltaTime)
    {
        ...
        rd.AddForce(new Vector3(0.0f, -PostionYforce * deltaTime, 0.0f));
    }
